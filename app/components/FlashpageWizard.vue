    ...
    <div v-else-if="currentStep === 1" class="space-y-8">
      <div class="space-y-6">
        <div class="mb-6">
          ...
          />
          <div class="mt-2 font-mono text-sm">{{ formData.content.length }}/1000 CHARACTERS</div>
        </div>
      </div>

      <div
        v-if="formData.content"
        class="rounded-lg bg-gradient-to-br from-purple-50 to-blue-50 p-6 shadow-sm transition-all hover:shadow-md"
      >
        <h3 class="mb-3 text-sm font-medium text-gray-700">Content Preview</h3>
        <p class="whitespace-pre-wrap text-gray-900">{{ formData.content }}</p>
      </div>

      <!-- GIF Picker -->
      <div class="space-y-4">
        <div class="border-t border-gray-200 pt-6">
          <h3 class="mb-4 text-lg font-semibold text-gray-900">Add a GIF (Optional)</h3>
    ...
    <div v-else-if="currentStep === 2" class="space-y-8">
      <div class="space-y-6">
        <div class="text-center">
          <h3 class="text-lg font-semibold text-gray-900 mb-2">Choose Your Theme</h3>
          <p class="text-gray-600">Pick colors and style that match your vibe</p>
        </div>

        <UFormField label="Color Theme" description="Choose a color scheme for your page">
          <URadioGroup
        ...
        <UFormField label="Dark Mode" description="Enable dark theme for your page">
      ...
      <div class="rounded-lg border border-gray-200 p-6 shadow-sm transition-all hover:shadow-md">
        <h3 class="mb-4 text-center text-sm font-medium text-gray-700">Theme Preview</h3>
        <div
          :class="[
      ...
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-6">
        ...
        <div class="space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="text-center">
              ...
              <p class="mt-1 capitalize">
                {{ formData.theme }} {{ formData.isDarkMode ? '(Dark)' : '(Light)' }}
              </p>
            </div>
          </div>
          ...
          <div class="text-center">
            ...
            <div class="mt-2 rounded-lg bg-white p-4">
          ...
          <div v-if="formData.gifUrl" class="text-center">
            ...
            <div class="mt-2 inline-block overflow-hidden rounded-lg border border-gray-200">
              <img :src="formData.gifUrl" alt="Selected GIF" class="h-32 w-auto object-cover" />
            </div>
          </div>
        </div>
      </div>

      <UAlert v-if="submitError" color="error" variant="subtle" :title="submitError"> </UAlert>
...
<script setup lang="ts">
  const { formData, submitError, isLoading, submitForm, checkAvailability, currentStep } =
    useFlashpageWizard()
